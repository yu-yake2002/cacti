    ...
    for (int d2 = d1; d2 < 6; d2++) {
      for (int d3 = d2; d3 < 6; d3++) {
        ...
        if (((current_dimm_model == JUST_LRDIMM) ||
          ...
          if (d1 > 0) num_dimm_per_channel++;
          dimm_cap.push_back(DIMM_size[d2]);
          if (d2 > 0) num_dimm_per_channel++;
          dimm_cap.push_back(DIMM_size[d3]);
          if (d3 > 0) num_dimm_per_channel++;
          ...
          for (int bw_id = 0; bw_id <= max_index; ++bw_id) {
            ...
            if ((DIMM_size[d1] + DIMM_size[d2] + DIMM_size[d3]) == 0) continue;
...
void find_mems_recursive(MemCadParameters* memcad_params,
  ...
  if (nb == 1) {
    for (unsigned int i = start; i < memcad_all_bobs->size(); ++i) {
      if ((*memcad_all_bobs)[i]->capacity != remaining_capacity) continue;
...
bool find_all_memories(MemCadParameters* memcad_params) {
  ...
  for (int i = 0; i < min_num_results; ++i) {
    if ((*memcad_all_memories)[i]) cout << *(*memcad_all_memories)[i] << endl;
  }
  return true;
}
